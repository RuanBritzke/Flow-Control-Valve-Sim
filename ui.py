...
PUMP_ASSETS = {
    "PUMP_NORMAL": "Pump-Unpressed.svg",
    "PUMP_PRESSED": "Pump-Pressed.svg",
}
    
class ValveSimulationUI(QMainWindow):
    ...
    def build_grid(self):
        self.grid.setColumnStretch(0, 1) # pump column
        self.grid.setColumnStretch(1, 2) # manifold column
        self.grid.setColumnStretch(2, 1) # valve column
        self.grid.setColumnStretch(3, 2) # info column

        #Header row
        self.grid.addWidget(self.debug_box("pump"), 0, 0)
        self.grid.addWidget(self.debug_box("Common Close Manifold"), 0, 1)
        self.grid.addWidget(self.debug_box("TRFC"), 0, 2)
        self.grid.addWidget(self.debug_box("Information"), 0, 3)

        # First valve blocks
        pump_widget = PumpWidget(
    ...
    def render_svg_to_pixmap(self, path: Path, width: int, height: int) -> QPixmap:
